...
function ModalImpl({ resolvePromise, options = {} }) {
  ...
  const {
    ...
    handleSubmit,
    reset,
    setValue, // <--- ADDED: Necessary to manually update category state for validation
    formState: { errors, isSubmitting },
  } = useForm({
    resolver: zodResolver(productSchema),
    defaultValues: { name: "", price: "", discountPrice: "", description: "", categories: [],  available: 0 },
  ...
  useEffect(() => {
   ...
   const categoryIds = Array.isArray(cats) 
      ? cats.filter(c => c).map(c => typeof c === 'object' ? c.id : c) 
      : [];
    ...
    const validCats = Array.isArray(cats) 
      ? cats.filter(c => c && typeof c === 'object') 
      : [];
  ...
  function toggleCategory(c) {
    setSelectedCats((prev) => {
      ...
      if (has) {
        newCats = prev.filter((p) => p.id !== c.id);
      } else {
        if (prev.length >= 3) return prev;
        newCats = [...prev, c];
      }

      setValue("categories", newCats.map(cat => cat.id), { shouldValidate: true });
  ...
  async function onSubmit(values) {
    ...
    try {
      ...
      const productPayload = {
        ...
        // Your DB expects relations via ID, not full objects.
  ...
  // Debug helper to catch silent validation failures
      ...
      >
        ...
        <form className="p-4 space-y-4">
          ...
          <div>
            ...
            {/* Added error feedback for categories */}
